...
Functions for loading NOAA ADIOS oil database, computing oil properties,
and managing particle configurations.
...
from particle import FluidMixture
from particle_functions import get_k_bio
...
def load_adios_oil(adios_id):
    ...
    # Extract the densities of each pseudo component
    ...
    density2 = density * (np.sum(density * mass_frac)) / (1 / np.sum(mass_frac / density))
    ...
    solubility = 46.4 * 10. ** (-36.7 * molecular_weight / density)
    #  solubility: Array of solubilities (mol/L) for each pseudo-component of the oil.

    k_h_0 = get_henry_constant(solubility, vapor_pressure_25C, molecular_weight)
    #  the Henry's Law constant at standard conditions for each pseudo-component (kg/(m^3 Pa))

    Tc, Pc, Vc, M, omega, delta = get_preos_params(boiling_point, molecular_weight, density)
    nu_bar = (-2.203e-5 * Pc + 518.6 * M + 143.4) * 1e-6
    neg_delta_H_sol_R = 2.637 * Tc + 22.48e6 * nu_bar + 314.6
    K_salt = (-1.345 * M + 2799.4 * nu_bar + 0.083556) / 1000.

    # Estimate Vb based on the Tyn and Calus formula
    Vb = (0.285 * (Vc * 1.e6) ** 1.048) * 1e-6
    ...
    user_data, units = format_dbm_data(composition, M, Pc, Tc, omega, k_h_0,
                                       neg_delta_H_sol_R, nu_bar, K_salt,
                                       Vc, boiling_point, Vb,
                                       B=None, dE=None)
    ...
    user_data = Vc_tuning(mass_frac, composition, T_0, rho_0, w_0, density2, delta, user_data)
...
def Vc_tuning(mass_frac, composition, T_0, rho_0, w_0, rho_i, delta, user_data):
    """
    ...
    def delta_rho(Vc, component, user_data, rho_i, T, P):
        ...
        """
        # Update the value of Vc in the user_data for the present oil component
        user_data[component]['Vc'] = Vc

        # Create a dbm.FluidMixture object for this oil component with the
        # present user data.
        oil_comp = FluidMixture(component, user_data=user_data)
    ...
    sum_abs = 0.
    sum_rel = 0.
    oil = FluidMixture(composition, user_data=user_data, delta=delta)
...
def natural_gas():
    ...
    # gas_compounds = ['methane', 'ethane', 'propane', 'isobutane', 'n-butane', 'isopentane', 'n-pentane']
    # gas_fractions = np.array([0.825, 0.083, 0.053, 0.0097, 0.0189, 0.0052, 0.0052])
...
def pedersen(M, composition=[]):
    ...
    # Compute each off-diagonal term using the correlation to hydrocarbon-hydrocarbon mixtures.
    ...
    if len(composition) > 0:
        ...
        # gas = ['methane', 'ethane', 'propane', 'isobutane', 'n-butane', 'isopentane', 'n-pentane']

        # Get the values of delta between air (rows) and the natural gases (columns)
        # delta_air_gas = np.array([
        #     [0.0311, 0.0515, 0.0852, 0.1033, 0.08, 0.0922, 0.1000],
        #     [0.0311, 0.0515, 0.0852, 0.1033, 0.08, 0.0922, 0.1000],
        ...
        for i in range(len(composition)):
            ...
            if not known:
                ...
                # compound in chems...set these binary interaction parameters to zero
                delta[i, :] = 0.
                delta[:, i] = 0.
...
def get_oil(substance, q_oil, gor=0, fp_type=1):
    composition, mass_frac, user_data, delta, delta_groups, units = load_adios_oil(substance)

    # Create a live oil mixture for this oil that has the given GOR
    if gor > 0:
        composition, mass_frac, delta = mix_gas_for_gor(composition, mass_frac, user_data, gor)

    # Get the mass flux for the desired oil flow rate
    mass_flux = set_mass_fluxes(composition, mass_frac, user_data, delta, q_oil, fp_type, gor)

    # Create the dbm.FluidMixture object
    oil = FluidMixture(composition, delta=delta, user_data=user_data)

    return oil, mass_flux


def set_mass_fluxes(composition, mass_frac, user_data, delta, q_oil, fp_type, gor):
    """
    ...
    if gor > 0:
        ...
        m0 = np.zeros((2, len(mass_frac)))
        m0[fp_type, :] = mass_frac

    # Get the volume flow rate of liquid oil at standard conditions for a
    # total petroleum fluid flow rate of 1 kg/s (e.g., using mass_flux equal to mass_frac)
...
def mix_gas_for_gor(dead_composition, dead_mass_frac, user_data, gor):
    """
    ...
    oil = FluidMixture(composition, user_data=user_data)
    delta = pedersen(oil.M, composition)
    # Create a new FluidMixture object with the correct binary interaction matrix
    oil = FluidMixture(composition, delta=delta, user_data=user_data)
    ...
    # Iterate to converge on a correct gas fraction to have the desired gas to oil ratio
